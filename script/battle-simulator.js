...
class BattleSimulator {
  ...
  async startBattle(pokemon1, pokemon2) {
    console.log(
      "[Battle] Starting battle between",
      pokemon1.name,
      "and",
      pokemon2.name,
    );
    ...
    console.log(
      "[Battle] Battle initialized with moves:",
  ...
  getDefaultMoves(pokemonDetails) {
    ...
    const typeMove =
      defaultMovesByType[primaryType] || defaultMovesByType["normal"];
    const tackle = {
      name: "tackle",
      displayName: "TACKLE",
      power: 40,
  ...
  renderFighter(fighter, fighterId) {
    ...
    const fighterClass =
      this.currentBattle.winner === fighterId
        ? "winner"
        : fighter.currentHp <= 0
          ? "defeated"
          : "";
  ...
  renderBattleLog() {
    return this.battleLog
      .map((entry) => {
        ...
        return `<div class="log-entry ${entry.type}">${safeMessage}</div>`;
      })
      .join("");
  }
  ...
  playRound() {
    ...
    const firstAttacker =
      fighter1.stats.speed >= fighter2.stats.speed
        ? {
            attacker: fighter1,
            defender: fighter2,
            attackerId: "fighter1",
    ...
    const secondAttacker =
      firstAttacker.attackerId === "fighter1"
        ? {
            attacker: fighter2,
            defender: fighter1,
            attackerId: "fighter2",
    ...
    this.executeAttack(
      ...
      firstAttacker.attackerId,
      firstAttacker.defenderId,
    );
    ...
    if (secondAttacker.defender.currentHp > 0) {
      setTimeout(() => {
        this.executeAttack(
          ...
          secondAttacker.attackerId,
          secondAttacker.defenderId,
        );
  ...
  executeAttack(attacker, defender, attackerId, defenderId) {
    ...
    if (hitChance > move.accuracy) {
      this.addLog(
        `Die Attacke von ${attacker.name} verfehlt das Ziel!`,
        "miss",
      );
      return;
    }
    ...
    // Battle Log
    this.addLog(
      `${defender.name} erleidet ${damageResult.damage} Schaden!`,
      "damage",
    );

    // Critical Hit
    if (damageResult.isCritical) {
      this.addLog("Ein Volltreffer!", "critical");
    }

    // Typ-Effektivität anzeigen
    if (damageResult.effectiveness > 1) {
      this.addLog("Es ist sehr effektiv!", "effectiveness");
    } else if (
      damageResult.effectiveness < 1 &&
      damageResult.effectiveness > 0
    ) {
      this.addLog("Es ist nicht sehr effektiv...", "effectiveness");
    } else if (damageResult.effectiveness === 0) {
      this.addLog("Es hat keine Wirkung auf den Gegner...", "no-effect");
    }
    ...
    this.requestDynamicBattleCommentary({
      attacker,
      defender,
      move,
      damageResult,
  ...
  async requestDynamicBattleCommentary({
    attacker,
    defender,
    move,
    damageResult,
  }) {
    const fallbackComment = this.buildLocalBattleCommentary({
      attacker,
      defender,
      move,
      damageResult,
    ...
    if (
      !aiService ||
      typeof aiService.requestBattleCommentary !== "function" ||
      typeof aiService.hasGroqKey !== "function" ||
      !aiService.hasGroqKey()
    ) {
      this.addLog(fallbackComment, "commentary");
      return;
    }

    if (this.pendingCommentaryRequests >= this.maxPendingCommentaryRequests) {
      this.addLog(fallbackComment, "commentary");
      return;
    }

    this.pendingCommentaryRequests += 1;
    const effectivenessText = this.describeEffectiveness(
      damageResult.effectiveness,
    );

    try {
      const aiComment = await aiService.requestBattleCommentary({
        attackerName: attacker.name,
        moveName: move.displayName || move.name || "Attack",
        defenderName: defender.name,
        effectiveness: effectivenessText,
      });
      this.addLog(aiComment || fallbackComment, "commentary");
    } catch (error) {
      console.warn(
        "[Battle] AI commentary failed, using local fallback",
        error,
      );
      this.addLog(fallbackComment, "commentary");
    } finally {
      this.pendingCommentaryRequests = Math.max(
        0,
        this.pendingCommentaryRequests - 1,
      );
  ...
  buildLocalBattleCommentary({ attacker, defender, move, damageResult }) {
    const moveName = move.displayName || move.name || "Attack";
    const effectivenessText = this.describeEffectiveness(
      damageResult.effectiveness,
    );

    if (damageResult.isCritical) {
      return `${attacker.name} landet mit ${moveName} einen brutalen Volltreffer gegen ${defender.name}!`;
    }

    if (damageResult.effectiveness > 1) {
      return `${moveName} trifft ${defender.name} mit voller Wucht, das war ${effectivenessText}!`;
    }

    if (damageResult.effectiveness === 0) {
      return `${attacker.name} feuert ${moveName} ab, doch ${defender.name} bleibt unbeeindruckt!`;
    }

    if (damageResult.damage <= 12) {
      return `${defender.name} steckt ${moveName} weg und bleibt kampfbereit!`;
    }

    return `${attacker.name} setzt ${moveName} ein und zwingt ${defender.name} in die Defensive!`;
  }

  describeEffectiveness(effectiveness) {
    if (effectiveness === 0) return "wirkungslos";
    if (effectiveness > 1) return "sehr effektiv";
    if (effectiveness < 1) return "nicht sehr effektiv";
  ...
  exportBattleLog() {
    ...
    const exportData = {
      metadata: {
        ...
        fighter1: fighter1
          ? { id: fighter1.id, name: fighter1.name, types: fighter1.types }
          : null,
        fighter2: fighter2
          ? { id: fighter2.id, name: fighter2.name, types: fighter2.types }
          : null,
        rounds: this.roundCounter,
        winner: this.currentBattle?.winner || null,
      },
      battleLog: this.battleLog.map((entry) => ({
        type: entry.type,
